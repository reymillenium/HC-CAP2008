...
uses
  ...
  UDptoCapacitacion, UPuestoDeTrabajo,
  UDataModuleControlDelSistema, UPantalla;
...
begin
  ...
  if MessageDlg('¿Desea cancelar la validación en el sistema HC-CAP 2008? La aplicación se cerrará.', mtConfirmation, [mbYes, mbNo], 0) = mrYes
    then
      begin
          ...
          if (   (lbledtUsuario.Text<> '') and (lbledtContrasenna.Text<> '')   )
            then
              begin
                ...
                try
                DptoCapacitacion.EstablecerConexionConLaBD(ContrasennaBD, UsuarioBD, NombreBaseDeDatos, NombreServer, Intranet);
                except
                on Exception do
                ...
                if (DptoCapacitacion.ExisteConexionConLaBD = True)
                  then
                    begin
                      if (DptoCapacitacion.ExisteElPuestoDeTrabajoConUsuario(usuario))
                        then
                          begin
                            //Aquí es donde debo verificar si existe alguien loggeado en el sistema HCCAP
                            if (DptoCapacitacion.HayAlguienLoggeado = False)
                              then
                                begin
                                  DptoCapacitacion.BuscarPuestoDeTrabajo(usuario);

                                  //Verifico si la contraseña coincide
                                  if (DBTextContrasenna.Field.Text = DptoCapacitacion.EncriptarSeudoaleatorio(contrasenna))
                                    then
                                      begin
                                        //Busco al usuario como si fuese Administrador, para luego comprobar si lo halló
                                        DptoCapacitacion.SelecAdministrador(usuario);
                                        ...
                                        DptoCapacitacion.LoggearAlUsuario(usuario, Privilegio, NombreUsuario, PrimerApellido, SegundoApellido);
                                    ...
                                    begin // Hay alguien supuestamente loggeado. Debo verificar si aun lo está (posible apagón)
                                      ...
                                      frmValidacionPuestoDeTrabajo.Caption:= 'HC-CAP 2008. Verificando conexión';
                                      ...
                                      DptoCapacitacion.BuscarAlLoggeado;
...
begin
  if (Key in [#13]) then btnEntrar.Click;
...
var 
  hMenuHandle : HMENU;
begin
  frmValidacionPuestoDeTrabajo.Caption:= 'HC-CAP 2008. Validación de Entrada';
  ...
  if (DptoCapacitacion.ExisteConexionConLaBD)
    then
      begin
        ...
        DptoCapacitacion.ActualizarPermanencia;
      end;
end;
      ...
      begin //Hay cambios (hay permanencia)
        ...
        Application.Terminate;
          ...
          begin //No hay cambios  (no hay permanencia = apagón)
            ...
            DptoCapacitacion.BorrarFalsoLoggeo;
